        ...
        firestore?.collection("users")?.document(uid!!)?.addSnapshotListener { value, error ->
            ...
            if(followDTO?.followerCount != null){
                ...
                if(followDTO?.followers?.containsKey(currentUserUid!!)){
                    if(isAdded)fragmentView?.account_btn_follow_signout?.text = getString(R.string.follow_cancel)
                    if(isAdded)fragmentView?.account_btn_follow_signout?.background?.setColorFilter(ContextCompat.getColor(activity!!,R.color.colorLightGray),PorterDuff.Mode.MULTIPLY)

                }else{
                    if(uid != currentUserUid){
                        if(isAdded)fragmentView?.account_btn_follow_signout?.text = getString(R.string.follow)
                        if(isAdded)fragmentView?.account_btn_follow_signout?.background?.colorFilter = null
                    }
